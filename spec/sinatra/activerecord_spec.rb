...
RSpec.describe "the sinatra extension" do
  ...
  xit "allows settings environments as Strings" do
    app.environment = "development"
    app.database = {development: {adapter: "sqlite3", database: "tmp/foo.sqlite3"}}
  ...
  xit "raises an appropriate error when the database spec is invalid" do
    expect{app.database = {}}.to raise_error(ActiveRecord::AdapterNotSpecified)
  end

  xit "doesn't try to establish connection when database isn't set" do
    expect{app.database}.not_to raise_error
  end

  xit "establishes connection from DATABASE_URL if it's present" do
  ...
  xit "allows specifying database through a file" do
    app.database_file = "spec/fixtures/database.yml"

    expect{ActiveRecord::Base.connection}.not_to raise_error
  end

  xit "allows specifying multiple databases" do
  ...
  xit "allows specifying multiple databases, with URL having precedence" do
    ENV["DATABASE_URL"] = database_url
  ...
  xit "expands database file path from the app root if present" do
    app.root = "spec/fixtures"
    app.database_file = "database.yml"
  ...
  xit "doesn't expand the database file path from the app root if the path is absolute" do
    app.root = "spec/fixtures"
  ...
  xit "raises an error on invalid database.yml" do
  ...
  xit "raises an error on missing database.yml" do
